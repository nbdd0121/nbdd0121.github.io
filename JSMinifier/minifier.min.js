(function e(t, n, r) {
					...
					case 2:
						{
							...
							return pageMap[ch & 255];
						}
					case 14:
						{
							var pageMap = planeE_type_map[ch >> 8 & 255];
							if (pageMap.length == 1) {
								return pageMap;
							}
							return pageMap[ch & 255];
		...
		module.exports = function() {
			...
			Grammar.prototype.expectSemicolon = function() {
				...
				if (nxt.type == '}' || nxt.lineBefore) {
					...
					throw new SyntaxError('Expected semicolon after statement');
				}
			};
			...
			Grammar.prototype.arrayLiteral = function() {
				...
				while (true) {
					...
					if (lh.type == ']') {
						...
						throw new SyntaxError('Expected comma or right bracket in array initializer expression');
					}
					if (this.lookahead().type == ']') {
						this.next();
						return ret;
			...
			Grammar.prototype.objectLiteral = function() {
				...
				while (true) {
					...
					if (lh.type == '}') {
						...
						throw new SyntaxError('Expected comma or right brace in object initializer expression');
					}
					this.lex.parseId = false;
			...
			Grammar.prototype.objectLiteralProp = function() {
				...
				if (name.type != 'str' && name.type != 'num' && name.type != 'id') {
					throw new SyntaxError('Expected identifier in object initializer');
				}
				var colon = this.next();
				this.lex.parseId = true;
				if (colon.type == ':') {
					...
					if (name.type != 'id' || name.value != 'get' && name.value != 'set') {
						throw new SyntaxError('Expected colon after identifier in object initializer');
					}
					if (name.value == 'get') {
			...
			Grammar.prototype.caseBlock = function() {
				...
				while (true) {
					...
					if (lhd.type == 'case') {
						...
						throw new SyntaxError('Expected case clause or default clause');
					}
			...
			Grammar.prototype.throwStmt = function() {
				...
				if (this.lookahead().lineBefore) {
					throw new SyntaxError('Expected Expression in throw statement');
				}
	...
	'5': [function(require, module, exports) {
		...
		NorlitJSCompiler.minify = function(program) {
			var ast = NorlitJSCompiler.Parser(program);
			...
			function stateDefault() {
				...
				switch (next) {
					...
					case BOM:
						return null;
					case CR:
						if (this.lookahead() == LF) this.next();
					case LF:
					case LS:
					case PS:
						this.line++;
	...
	'7': [function(require, module, exports) {
							...
							default:
								{
									...
									if (code < 255) {
										var escape = '\\x' + (256 + code).toString(16).substr(1).toUpperCase();
									} else {
										var escape = '\\u' + (65536 + code).toString(16).substr(1).toUpperCase();
									}
									single += escape;
									double += escape;
									break;
		...
		var precedent = ['PrimaryExpression', '.', 'LeftHandSideExpression', 'PostfixExpression', 'UnaryExpression', '*', '+', '<<', '<', '==', '&', '^', '|', '&&', '||', 'ConditionalExpression', 'AssignmentExpression', 'Expression'];
			...
			if (ast instanceof Object) {
				switch (ast.type) {
					...
					case 'MemberExpression':
						{
							if (typeof ast.property == 'string' && isIdentifierName(ast.property)) {
								return {
									str: wrap(minify(ast.base), 'LeftHandSideExpression').str + '.' + ast.property,
									p: '.'
								};
							} else {
								return {
									str: wrap(minify(ast.base), 'LeftHandSideExpression').str + '[' + minify(ast.property, 'Expression').str + ']',
									p: '.'
								};
							}
						}
					...
					case 'ConditionalExpression':
						{
							return {
								str: wrap(minify(ast.test), '||').str + '?' + wrap(minify(ast.true), 'AssignmentExpression').str + ':' + wrap(minify(ast.false), 'AssignmentExpression').str,
					...
					case 'ForStatement':
						{
							...
							if (ast.init !== undefined) {
								if (ast.init && ast.init.type == 'VariableDeclaration') str += minify(ast.init).str;
								else str += minify(ast.init).str + ';';
							} else {
								str += ';';
							}
					...
					case 'ForInStatement':
						{
							var str = 'for(';
							if (ast.init && ast.init.type == 'VariableDeclarator') str += 'var ' + minify(ast.var);
							else str += minify(ast.var).str;
							return {
					...
					case 'TryStatement':
						{
							...
							if (ast.catch !== undefined) {
								str += 'catch(' + ast.parameter + ')' + minify(ast.catch).str;
							}
					...
					case 'VariableDeclarator':
						{
							if (ast.init !== undefined) {
								return {
									str: ast.name + '=' + wrap(minify(ast.init), 'AssignmentExpression').str
								};
							} else {
								return {
									str: ast.name
